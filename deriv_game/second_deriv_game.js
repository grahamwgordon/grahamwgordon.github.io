...
function draw() {
    ...
    if (game_state == 0) {
        ...
        // draw second deriv_points
        //_draw_a_graph(deriv_points,0,0,222) ;
        _draw_a_graph(second_deriv_points,0,0,222) ;
...
function mousePressed() {
    if (game_state == 0) {
        ...
        second_deriv_points = [] ;
        second_deriv_points.push([0,height/2]) ;
...
function _create_new_graph_points() {
    ...
    for (var i=0; i<width/gstep; i++) {
        ...
        //current_y += current_d ;
        current_y = height/2 + (2*noise(i*noiseScale)-1)*height/4 ;
...
function _set_second_deriv_points() {
    second_deriv_points = [] ;
    second_deriv_points.push([0,height/2]) ;
    for (var i=1; i<graph_points.length; i++) {
        second_deriv_points.push([i*gstep,height/2+unit*(deriv_points[i][1]-deriv_points[i-1][1])/(deriv_points[i][0]-deriv_points[i-1][0])]) ;
    }
}
...
function difference_o_meter() {
    var tot = 0 ;
    for (var i=0; i<second_deriv_points.length; i++) {
        tot += (gstep / unit) * abs(drawn_points[i][1] - second_deriv_points[i][1]) ;
    }
